...
struct FileDecode {
    ...
    FILE *outdecodedYUVfile = nullptr;
    int videoStream = -1;
    int audioStream = -1;
    ...
    bool open(string filename) {
        ...
        audioStream = av_find_best_stream(formatCtx, AVMEDIA_TYPE_AUDIO, -1, -1,
                                          nullptr, 0);
        videoStream = av_find_best_stream(formatCtx, AVMEDIA_TYPE_VIDEO, -1, -1,
                                          NULL, 0);
    ...
    int OpenAudioDecode() {
        ...
        // 将参数从 codecpar 拷贝到 audioCodecCtx
        if (avcodec_parameters_to_context(audioCodecCtx, codecPara) < 0) {
    ...
    bool DecodeAudio(AVPacket *originalPacket) {
        int res = avcodec_send_packet(audioCodecCtx, originalPacket);
        ...
        res = avcodec_receive_frame(audioCodecCtx, frame);
        if (res == AVERROR_EOF || res == AVERROR(EAGAIN)) {
            print_av_error(res);
            av_frame_free(&frame);
            return true;
        ...
        if (av_sample_fmt_is_planar(audioCodecCtx->sample_fmt)) {
            ...
            fwrite(frame->data[0], 1,
                   data_size * frame->nb_samples * audioCodecCtx->channels,
                   outdecodedfile);
        }
    ...
    bool OpenVideoDecode() {
        AVCodecParameters *params = formatCtx->streams[videoStream]->codecpar;
        const AVCodec *codec = avcodec_find_decoder(params->codec_id);
        if (codec == nullptr) {
            std::cout << "cannot find video codec id: " << params->codec_id <<
                std::endl;
            return -1;
        }
        videoCodecCtx = avcodec_alloc_context3(codec);
        if (videoCodecCtx == nullptr) {
            std::cout << "cannot alloc video codec ctx" << std::endl;
            return -1;
        }
        if (avcodec_parameters_to_context(videoCodecCtx, params) < 0) {
            std::cout << "cannot copy video codec params to ctx" << std::endl;
        }
    ...
    bool DecodeVideo(AVPacket *originalPacket) {
        int res = avcodec_send_packet(videoCodecCtx, originalPacket);
        if (res < 0) {
            print_av_error(res);
            return false;
        }
        ...
        while (true) {
            ...
            res = avcodec_receive_frame(videoCodecCtx, frame);
            if (frameCount++ == 23 * 10) {
                break;
            }
            if (res == AVERROR_EOF || res == AVERROR(EAGAIN)) {
            ...
            auto is_planar_yuv = [](AVPixelFormat pix_fmt) -> bool {
                switch (pix_fmt) {
                case AV_PIX_FMT_YUV420P:
                case AV_PIX_FMT_YUV422P:
                case AV_PIX_FMT_YUV444P:
                    // Add more planar pixel formats here if needed
                    return true; // Planar layout
                default: return false; // Non-planar layout
                }
            };
            ...
            const int ysize = frame->linesize[0];
            const int usize = frame->linesize[1];
            const int vsize = frame->linesize[2];
            const int width = frame->width;
            const int height = frame->height;

            if (pix_fmt == AV_PIX_FMT_YUV420P) {
                // fwrite(frame->data[0], 1, width * height,
                //        outdecodedYUVfile); // Y
                // fwrite(frame->data[1], 1, width * height / 2,
                //        outdecodedYUVfile); // U
                // fwrite(frame->data[2], 1, width * height / 2,
                //        outdecodedYUVfile); // V
                for (int i = 0; i < height; ++i)
                    fwrite(frame->data[0] + i * frame->linesize[0], 1, width,
                           outdecodedYUVfile);

                // Write U plane (subsampled)
                for (int i = 0; i < height / 2; ++i)
                    fwrite(frame->data[1] + i * frame->linesize[1], 1,
                           width / 2, outdecodedYUVfile);

                // Write V plane (subsampled)
                for (int i = 0; i < height / 2; ++i)
...
int main(int argc, char *argv[]) {
    //change workding dir to CURRENT_DIRECTORY
    ...
    if (fileDecode.OpenVideoDecode() < 0) {
        return -1;
    }
